...
const Results = ({ results, onClear }: { results: ScanResults; onClear: () => void }) => {
  ...
  const stats = React.useMemo(() => {
    ...
    return results.findings.reduce(
      (acc, vuln) => {
        ...
        const key = severity.toLowerCase() as keyof typeof acc;
        if (key in acc && key !== "total") acc[key]++;
        return acc;
      },
  ...
  return (
    <div className="max-w-[900px] mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={onClear} className="px-4 py-2 bg-secondary text-secondary-foreground border border-border rounded-md font-medium hover:bg-muted transition-colors">New Scan</button>
      </div>
      ...
      {results.metadata && (
        <div className="bg-muted border border-border rounded-lg p-4 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {results.metadata.scan_duration && <div><span className="text-xs text-muted-foreground">Duration: </span><span className="text-sm font-medium">{results.metadata.scan_duration.toFixed(2)}s</span></div>}
            {results.metadata.files_scanned && <div><span className="text-xs text-muted-foreground">Files Scanned: </span><span className="text-sm font-medium">{results.metadata.files_scanned}</span></div>}
            {results.metadata.rules_applied && <div><span className="text-xs text-muted-foreground">Rules Applied: </span><span className="text-sm font-medium">{results.metadata.rules_applied}</span></div>}
            {results.metadata.source_url && <div><span className="text-xs text-muted-foreground">Source: </span><span className="text-sm font-medium">{results.metadata.source_url}</span></div>}
          </div>
        </div>
      )}
      <div className="flex justify-between items-center mb-4 flex-wrap gap-3">
        <div className="flex gap-2 flex-wrap">
          <FilterButton label="All" isActive={activeFilter === "ALL"} onClick={() => setActiveFilter("ALL")} count={stats.total} />
          <FilterButton label="Critical" isActive={activeFilter === "CRITICAL"} onClick={() => setActiveFilter("CRITICAL")} count={stats.critical} />
          <FilterButton label="High" isActive={activeFilter === "HIGH"} onClick={() => setActiveFilter("HIGH")} count={stats.high} />
          <FilterButton label="Medium" isActive={activeFilter === "MEDIUM"} onClick={() => setActiveFilter("MEDIUM")} count={stats.medium} />
          <FilterButton label="Low" isActive={activeFilter === "LOW"} onClick={() => setActiveFilter("LOW")} count={stats.low} />
        </div>
        <input type="text" placeholder="Search vulnerabilities..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="px-3 py-2 border border-border rounded-md text-sm w-[250px] bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
      </div>
      <div className="flex gap-3 mb-4">
        <button onClick={() => setExpandedIds(new Set(filteredVulnerabilities.map((_, i) => i)))} className="text-sm text-primary hover:underline">Expand All</button>
        <button onClick={() => setExpandedIds(new Set())} className="text-sm text-primary hover:underline">Collapse All</button>
      </div>
      {filteredVulnerabilities.length > 0 ? (
        <div>{filteredVulnerabilities.map((vuln, index) => <VulnerabilityCard key={`${vuln.rule_id}-${vuln.file}-${vuln.line}-${index}`} vulnerability={vuln} isExpanded={expandedIds.has(index)} onToggle={() => toggleExpanded(index)} />)}</div>
      ) : (
        <div className="text-center p-12 bg-muted rounded-lg border border-border">
          <svg className="w-12 h-12 text-green-600 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          <h3 className="text-lg font-semibold text-foreground mb-2">{activeFilter !== "ALL" || searchQuery ? "No matching vulnerabilities" : "No vulnerabilities found"}</h3>
          <p className="text-muted-foreground">{activeFilter !== "ALL" || searchQuery ? "Try adjusting your filters or search query." : "Your code looks secure based on the applied rules."}</p>
        </div>
      )}
    </div>
  );
          ...
          <>
            <div className="bg-card border-b border-border">
              <div className="max-w-[600px] mx-auto flex justify-center">
                <Tab label="File Scan" icon={fileIcon} isActive={activeTab === "file"} onClick={() => setActiveTab("file")} />
                <Tab label="URL Scan" icon={urlIcon} isActive={activeTab === "url"} onClick={() => setActiveTab("url")} />
              </div>
            </div>
            <div className="py-8 pb-12">
              {activeTab === "file" ? <FileScan onScanComplete={handleScanComplete} /> : <UrlScan onScanComplete={handleScanComplete} />}
            </div>
          </>
        )}
      </main>
